...
void StatusBar::SetSize(float width, float height)
{
    ...
    // Keep the current fill ratio when the total size changes.
...
void StatusBar::SetValue(float currentValue, float maxValue)
{
    ...
    // Clamp the ratio so negative HP or overheal does not draw incorrectly.
    if (_ratio < 0.f)
    {
        _ratio = 0.f;
    }
...
UIManager::UIManager()
{
    // Player health bar: large, red, fixed in the top-left HUD.
    ...
    // Player experience bar: thinner and blue, below the health bar.
    ...
    // Enemy health bar: small, red, drawn above each enemy.
